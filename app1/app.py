...
COLOR = "#007bff"   # blue
CONTAINER_NUMBER = 1
CONTAINER_ID = os.getenv('HOSTNAME', 'container-1')
...
def home():
    ...
    if request_counter <= MAX_REQUESTS:
        ...
        <body>
            ...
            <div class="request-info">Next request will go to Container #{2 if CONTAINER_NUMBER == 1 else 3 if CONTAINER_NUMBER == 2 else 1}</div>
        </body>
        </html>
        """
    else:
        # Reset counter and indicate should redirect (nginx handles this)
        request_counter = 0
        return jsonify({
            "app": APP_NAME,
        ...
        }), 503  # Service Unavailable - triggers nginx to try next upstream
