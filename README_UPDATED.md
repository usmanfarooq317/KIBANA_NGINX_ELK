...
## **Fluent Bit & Kibana Updates**

- **Issue fixed:** The `fluent-bit` container previously failed to start because the compose mount attempted to map a host directory onto a container file (`/fluent-bit/etc/parsers.conf`). This caused an OCI runtime mount error.

- **What changed:**
  - `docker-compose.yml` updated to mount the config and scripts directories (instead of individual files or the whole `/fluent-bit` directory):
    - `./fluent-bit/etc:/fluent-bit/etc`
    - `./fluent-bit/scripts:/fluent-bit/scripts`
  - Added the following files on the host so the container reads them from `/fluent-bit/etc`:
    - `fluent-bit/etc/fluent-bit.conf` (Fluent Bit configuration)
    - `fluent-bit/etc/parsers.conf` (parsers file)
  - Added a JSON parser in `fluent-bit/etc/parsers.conf`:
    ```text
    [PARSER]
        Name   json
        Format json
...
- **Why this works:** Mounting only `./fluent-bit/etc` preserves the image's `/fluent-bit/bin` (where the Fluent Bit binary lives). Mounting individual files or the whole `./fluent-bit` directory risked overwriting image paths and making `/fluent-bit/bin/fluent-bit` unavailable.

- **Commands to verify and use:**
  ```bash
  # Rebuild and start services after fixes
  cd /home/usman/Desktop/Nginx-ELK
  docker compose up --build -d

  # Restart fluent-bit after adding parser (if needed)
  docker compose restart fluent-bit

  # Check fluent-bit logs
  docker compose logs -f fluent-bit

  # Check Elasticsearch indices (should see flask-YYYY.MM.DD)
  curl -sS http://localhost:9200/_cat/indices?v

  # Inspect a few documents to confirm structured fields
  curl -sS 'http://localhost:9200/flask-*/_search?size=3&sort=@timestamp:desc&pretty'
  ```

- **Create Kibana index pattern (example):**
  ```bash
  curl -sS -X POST "http://localhost:5601/api/saved_objects/index-pattern/flask-*" \
    -H "kbn-xsrf: true" \
    -H "Content-Type: application/json" \
    -d '{"attributes":{"title":"flask-*","timeFieldName":"@timestamp"}}'
  ```

- **Notes about parsed vs. raw logs:**
  - New log lines are parsed into structured fields (e.g. `appname`, `message`, `host`, `request_counter`).
  - Older documents ingested before the JSON parser may contain a `log` string with raw JSON. Reindexing with an ingest pipeline can convert those into fields if needed.

- **Cleanup suggestion:**
  - Remove any leftover/conflicting `fluent-bit/parsers.conf` directory to avoid confusion:
    ```bash
    rm -rf fluent-bit/parsers.conf
    ```

- **Next steps I can help with:**
  - Create a Kibana dashboard with visualizations for `flask-*` logs.
  - Reindex older logs into structured fields using an ingest pipeline.
  - Commit the compose and `fluent-bit/etc` config changes to git.
